...
class LabelClassifier:
    ...
    def _preprocess(self, image):
        preprocessed_image = self._preprocess_clip(
            self._preprocess_crop(image))
        with torch.no_grad():
            features_tensor = self._preprocess_clip(
                image).flatten()
    ...
    def _train(self):
        ...
        front, back = os.listdir(
            base_path + "front/"), os.listdir(base_path + "back/")
        ...
        images = [self._preprocess_crop(Image.open(path))
                  for path in image_paths]

        self.clip_model, self._preprocess_clip = clip.load(
            "ViT-B/32", device="cpu")
        self.clip_model.eval()

        with torch.no_grad():
            features_tensors = [self._preprocess_clip(
                image).flatten() for image in images]
            features = np.vstack([tensor.numpy()
                                 for tensor in features_tensors])
        ...
        def monitor(iteration, estimator, args):
            if iteration == 0:
                ...
                remaining_time = elapsed_time / \
                    (iteration + 1) * (estimator.n_estimators - iteration - 1)
                print(
    ...
    def images_are_inverted(self, image_a, image_b):
        features_a, features_b = self._preprocess(
            image_a), self._preprocess(image_b)
